...
module.exports = class Cluster {
  ...
  static enable(servers, cores) {
    if (Cluster._needBalancer(servers)) {
      ...
      if (servers.https.server) {
        webServers[servers.https.port] = {};
        webServers[servers.https.port].protocol = 'https';
        webServers[servers.https.port].server = servers.https.server;
      }
